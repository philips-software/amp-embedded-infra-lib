...
peer of a certificate to be used. A party will verify each certificate presented in this way by
...
SEC1 paragraph 2.3.3. It then uses ECDSA on the same SECP256R1 curve to compute the signature values r and s.
...
Upon reception, the peer uses the received r and s values to check the signature of the presented public key.
...
way: HMAC-DRBG-SHA256 is seeded with the shared secret, and generates 4 times 256 bits of data. The first 256
