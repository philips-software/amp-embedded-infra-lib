...
which specifies the available window size, encoded in big endian order. An Init packet must be responded to with an InitResponse
...
has one 16-bit unsigned parameter `size` which advertises available buffer size.
...
has one 16-bit unsigned parameter `size` which specifies the amount of bytes in the buffer
...
window sizes bigger than 4. When determining the buffer size needed to exchange packets, the
size of the biggest packet should be increased by 4 to accommodate for a ReleaseWindow packet.
...
----
(def boxes-per-row 4)
(def column-labels ["0" "1" "..." "n"])
(draw-column-headers)
(draw-box 4)
(draw-box "message" {:span 3})
----
...
== Appendix: MessageCommunicationSecurity.proto

[source,protobuf]
----
include::example$MessageCommunicationSecurity.proto[]
----
