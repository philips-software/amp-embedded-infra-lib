...
----
(def boxes-per-row 6)
(draw-column-headers {:labels ["0" "1" "2" "3" "..." "n"]})
(draw-box 1)
(draw-box "size" {:span 2})
(draw-box "init info" {:span 3})
----
...
Optionally, an `Init` packet can contain initialization information. This information is sent to its peer, and can for
example be used to send version information.
...
An `InitResponse` packet consumes 5 window bytes, plus any bytes for sending the init info: 3 for the packet contents, 0 or more for init info, 2 for the 
COBS layer (1 for the COBS overhead byte, one for the terminating 0).

Similar to the `Init` packet, the `InitResponse` packet may contain extra information.
...
3. Withholding a `ReleaseWindow` under rule 1 is only safe while the peer still has enough window
left to send a message. Once the window left to the peer drops below the size of one message plus
a `ReleaseWindow` packet, any freed up buffer space must be advertised immediately. Otherwise the
peer is unable to send anything, including the `ReleaseWindow` packet that would free up window on
this side, and the protocol deadlocks. This situation arises when a host holds on to received
packets while processing them, since that buffer space is not yet available for advertisement.
4. If enough buffer space is available, a sender should avoid consuming the entire peer's
...
a cyphertext of size equal to the size of the message, plus a 16 byte MAC. Following the recommendation in
NIST SP800-38D section 5.2.1.1, the length of the IV is 12 bytes. Since in `GCM` IVs may
not be reused, the IV is incremented for each message to avoid using the same IV twice: After
each message, treat the IV as a 96-bit unsigned Big Endian number, and add 1 to it (discarding any overflow).
This way, the protocol can securely handle 2^96^ messages each of size at most 2^39^ bytes before a
...
with the shared secret, and generates 2 times 128 + 96 bits of data. The first 128
bits of data is Key~A~, the next 96 bits is IV~A~, then 128 bits for Key~B~, then 96 bits for IV~B~. Both parties perform a lexicographical compare
