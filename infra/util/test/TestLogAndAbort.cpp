...
TEST(LogAndAbortTest, log_and_abort_no_handler_does_nothing)
{
    ...
    infra::ExecuteLogAndAbortHook(nullptr, 0, "fool of a Took");
    infra::ExecuteLogAndAbortHook(nullptr, 0, "speak %s and enter", "friend");
...
TEST(LogAndAbortTest, log_and_abort_with_filename_prints_filename)
{
    ...
    testing::internal::CaptureStdout();
    // Manually calling hook to avoid aborting the test
    infra::ExecuteLogAndAbortHook("filename.cpp", 32, "speak %s and enter", "friend");
    std::string output = testing::internal::GetCapturedStdout();
