...
TEST(MultiFunctionTest, construct_with_function)
{
    ...
    infra::MultiFunction<void()> f([&]() { m.callback(); });
    f();
}
...
TEST(MultiFunctionTest, multi_function_construct_with_second)
{
    ...
    infra::MultiFunction<void()>::And<void(bool)> f([&](bool v) { m.callback(v); });
    f(true);
}
...
TEST(MultiFunctionTest, return_value)
{
    ...
    infra::MultiFunction<int()>::And<void(bool)> f([&]() { return m.callback(); });
    EXPECT_EQ(5, f());
}
