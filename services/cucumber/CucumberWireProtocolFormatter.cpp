...
namespace services
{
    ...
    void CucumberWireProtocolFormatter::FormatStepMatchResponse(infra::TextOutputStream::WithErrorPolicy& stream)
    {
        ...
        case CucumberStepStorage::StepMatchResult::Success:
            if (controller.storageMatch.step->HasStringArguments())
                CreateSuccessMessage(stream, controller.storageMatch.id, FormatStepArguments(*(parser.nameToMatch)), controller.storageMatch.step->SourceLocation());
            else
    ...
    void CucumberWireProtocolFormatter::AddStringValue(infra::JsonArrayFormatter& formatter, infra::BoundedConstString nameToMatch, uint32_t& argPos, int16_t& offset)
    {
        infra::JsonObjectFormatter subObject(formatter.SubObject());
        infra::StringOutputStream::WithStorage<1024> stringStream;
        for (std::size_t i = 1; nameToMatch[argPos + offset + i] != '\''; ++i)
    ...
    void CucumberWireProtocolFormatter::AddDigitValue(infra::JsonArrayFormatter& formatter, infra::BoundedConstString nameToMatch, uint32_t& argPos, int16_t& offset)
    {
        infra::JsonObjectFormatter subObject(formatter.SubObject());
        infra::StringOutputStream::WithStorage<10> digitStream;
        for (uint8_t i = 0; argPos + offset + i < nameToMatch.size() && (nameToMatch[argPos + offset + i] >= '0' && nameToMatch[argPos + offset + i] <= '9'); ++i)
    ...
    void CucumberWireProtocolFormatter::AddBooleanValue(infra::JsonArrayFormatter& formatter, infra::BoundedConstString nameToMatch, uint32_t& argPos, int16_t& offset)
    {
        infra::JsonObjectFormatter subObject(formatter.SubObject());
